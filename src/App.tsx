...
const LogoIcon = ({ size = 24, dark = true }: { size?: number; dark?: boolean }) => (
  <svg width={size} height={size} viewBox="0 0 32 32" fill="none" xmlns="http://www.w3.org/2000/svg">
    <rect width="32" height="32" rx="7" fill="currentColor" />
    <path d="M7 12L13.5 16L7 20" stroke={dark ? "black" : "white"} strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" />
    <line x1="16" y1="20" x2="24" y2="20" stroke={dark ? "black" : "white"} strokeWidth="2.5" strokeLinecap="round" />
  </svg>
);
const GitHubIcon = () => (
  ...
  return (
    <ThemeContext.Provider value={ctx}>
      <div className={`min-h-screen transition-colors duration-300 ${tk.bg} ${tk.text}`}>
        ...
        {/* ── Top Banner ── */}
        <div className={`sticky top-0 z-20 h-10 flex items-center justify-between px-6 border-b ${tk.dark ? "border-white/7 bg-white/2" : "border-black/7 bg-black/2"} backdrop-blur-sm`}>
          <div className="flex items-center gap-2">
            <span className={tk.dark ? "text-white" : "text-black"}>
              <LogoIcon size={22} dark={tk.dark} />
            </span>
            <span className={`text-xs font-semibold tracking-wide ${tk.textMuted}`}>Dev Toolkit</span>
          </div>
          <div className="flex items-center gap-1">
            <a
              href="https://github.com/hananbabar98/dev-toolkit"
              target="_blank"
              rel="noopener noreferrer"
              className={`flex items-center gap-1.5 h-6 px-3 rounded-md border text-xs font-semibold transition-all duration-150 ${tk.dark ? "border-white/10 text-white/50 hover:border-white/25 hover:text-white/80 bg-white/3 hover:bg-white/7" : "border-black/10 text-black/50 hover:border-black/25 hover:text-black/80 bg-black/2 hover:bg-black/5"}`}
            >
              <GitHubIcon />
              <StarIcon />
              Star
